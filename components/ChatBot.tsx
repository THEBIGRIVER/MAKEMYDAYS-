    ...
    <div className="fixed bottom-0 md:bottom-24 right-0 md:right-8 z-[300] w-full md:w-96 h-[100dvh] md:h-[650px] bg-brand-navy md:border md:border-white/10 shadow-3xl flex flex-col animate-in slide-in-from-bottom duration-500 md:rounded-[2.5rem] overflow-hidden backdrop-blur-3xl">
      <div className="p-6 border-b border-white/5 flex items-center justify-between bg-black/40 shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-brand-prime rounded-lg flex items-center justify-center text-white font-black italic text-sm shadow-lg shadow-brand-prime/20">M</div>
          <div>
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 block">Experience</span>
            <span className="text-xs font-bold text-white uppercase tracking-widest">AI Concierge</span>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className="w-2 h-2 rounded-full bg-brand-prime animate-pulse shadow-[0_0_10px_rgba(0,168,225,1)]" />
          <button 
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] px-5 py-3.5 text-sm leading-relaxed rounded-2xl shadow-xl ${
              m.role === 'user' 
                ? 'bg-brand-prime text-white font-medium rounded-tr-none' 
      ...
      <form onSubmit={handleSend} className="p-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] border-t border-white/5 flex gap-3 bg-black/40 shrink-0">
        <input
          ...
          className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm outline-none text-white placeholder:text-slate-600 focus:border-brand-prime transition-all"
        />
        <button 
          type="submit" 
          disabled={!input.trim() || isTyping} 
          className="w-11 h-11 bg-brand-prime text-white flex items-center justify-center rounded-xl shadow-lg shadow-brand-prime/20 hover:brightness-110 active:scale-95 disabled:opacity-30 disabled:grayscale transition-all"
        >
